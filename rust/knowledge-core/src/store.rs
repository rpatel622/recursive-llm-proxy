...
impl KnowledgeStore for InMemoryKnowledgeStore {
    fn upsert(&self, chunks: Vec<EmbeddedChunk>) -> Result<()> {
        let mut stored = self.chunks.write().map_err(|error| crate::KnowledgeError::Storage(error.to_string()))?;
    ...
    fn vector_search(&self, query: &[f32], limit: usize) -> Result<Vec<SearchHit>> {
        let stored = self.chunks.read().map_err(|error| crate::KnowledgeError::Storage(error.to_string()))?;
        let mut hits: Vec<SearchHit> = stored.iter().filter_map(|item| {
            cosine_similarity(query, &item.embedding).map(|score| SearchHit { chunk: item.chunk.clone(), score })
        }).collect();
...
mod tests {
    ...
    fn ranks_cosine_similarity() {
        ...
        let chunk = |id: &str, embedding| EmbeddedChunk { chunk: DocumentChunk { id: id.into(), document_id: "d".into(), ordinal: 0, text: id.into(), metadata: BTreeMap::new() }, embedding };
        store.upsert(vec![chunk("near", vec![1.0, 0.0]), chunk("far", vec![0.0, 1.0])]).unwrap();
        assert_eq!(store.vector_search(&[1.0, 0.0], 1).unwrap()[0].chunk.id, "near");
    }
}
