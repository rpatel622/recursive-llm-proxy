...
def stop_local_proxy() -> Tuple[str, Dict[str, Any]]:
    try:
        status = managed_proxy.stop()
        return "Proxy stopped", status
    except Exception as exc:
...
def local_proxy_status() -> Tuple[str, Dict[str, Any]]:
    ...
    message = "Proxy is running" if status.get("running") else "Proxy is not running"
...
def build_ui(default_proxy_url: str = "http://127.0.0.1:8000", default_api_key: str = "") -> Any:
    import gradio as gr

    with gr.Blocks(title="recursive-llm proxy admin") as demo:
        gr.Markdown("# recursive-llm proxy admin")
        gr.Markdown(
            "Configure and launch the local proxy below, or connect to an already-running proxy. "
            "Settings apply when **Start / restart proxy** is pressed."
        )
        ...
        with gr.Tab("Configuration"):
            ...
            with gr.Row():
                private_api_base = gr.Textbox(
                    label="Private OpenAI-compatible API base",
            ...
            with gr.Row():
                ...
                process_status_button = gr.Button("Refresh process status")
            process_message = gr.Textbox(label="Local process status", interactive=False)
            process_details = gr.JSON(label="Local process details")

        gr.Markdown("## Active proxy connection")
        with gr.Row():
        ...
        process_status_button.click(
            local_proxy_status, outputs=[process_message, process_details]
        )
        check_button.click(
            connection_status,
        ...
        with gr.Tab("Slot catalog"):
            ...
            load_button.click(load_catalog, inputs=[proxy_url, api_key], outputs=[catalog_status, catalog])
            save_button.click(
                save_catalog,
        ...
        with gr.Tab("Test request"):
            ...
            with gr.Row():
                routing_mode = gr.Dropdown(
                    choices=["auto", "explicit", "clarify_only"], value="auto", label="Routing mode"
                )
                slot_slug = gr.Textbox(label="Slot slug")
                workstream_slugs = gr.Textbox(label="Workstream slugs", placeholder="comma,separated")
            with gr.Row():
